/*****************************************************************************
*																			 *
*					PROJET Sécurité et Réseaux                               *
*																			 *
*	Damois Hugo, Duval Arnaud, LEFREVRE Noé, SENGCHANH Dylan				 *
*  	SOK Michael, WERNERT Sébastien					 						 *
*																			 *
*	15/06/2022																 *
*																			 *
******************************************************************************/

DROP DATABASE IF exists bddvigenere;
CREATE DATABASE bddvigenere;

use bddvigenere;

CREATE TABLE users(
    userId INT PRIMARY KEY NOT NULL AUTO_INCREMENT, -- Attention j'ai changé id pour userId !!!
    name TEXT,
    email TEXT,
    password TEXT,
    admin BOOL, -- à quoi sert un admin dans notre appli ??
    city TEXT,
    privatekey TEXT,
    publickey TEXT,
    n TEXT
);

CREATE TABLE message(
	messageId INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
    ciphertext TEXT,
    ciphertextReturn TEXT,
    senderId INT NOT NULL,
    receiverId INT NOT NULL,
    messageDate TEXT,
    /*messageRead BOOL,*/
	FOREIGN KEY(senderID) REFERENCES users(userID),
	FOREIGN KEY(receiverID) REFERENCES users(userID)
);

INSERT INTO users (name, email, password, admin, city, privatekey, publickey, n) VALUES ('user1', 'user@email.com', '$2b$10$20nUuPkH20tCfg.poVVtruy7k.NYuKc8mtCIHR4CQUkcI6ZVblWzK', 0, 'Paris','','','');
INSERT INTO users (name, email, password, admin, city, privatekey, publickey, n) VALUES ('admin', 'admin@email.com', '$2b$10$20nUuPkH20tCfg.poVVtruy7k.NYuKc8mtCIHR4CQUkcI6ZVblWzK', 1, 'Paris','','','');


select * from message;
create view ciphertextForReceiver as
SELECT ciphertext as msg, messageDate, receiver.privatekey as pvk,receiver.n as n, sender.publickey, message.receiverId, message.senderId
from message
join users sender on sender.userId = message.senderId
join users receiver on receiver.userId = message.receiverId;
select * from message;
create view ciphertextForSender as
SELECT ciphertextReturn as msg, messageDate, sender.privatekey as pvk2,sender.n as n, sender.publickey, message.receiverId, message.senderId from message
join users sender on sender.userId = message.senderId
join users receiver on receiver.userId = message.receiverId;

select * from ciphertextForReceiver;
select * from ciphertextForSender;
select * from users;
/*Table ZIP*/

select * from ciphertextForReceiver UNION select * from ciphertextForSender

